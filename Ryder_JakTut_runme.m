...
nx = beta.x >= x_min - 1e4 & beta.x <= x_max + 1e4;
ny = beta.y >= y_min - 1e4 & beta.y <= y_max + 1e4; 
% 
...
% Interpolate to mesh
md_beta					= InterpFromGridToMesh(ss_x,ss_y,ss_beta,md.mesh.x,md.mesh.y,0);
...
md_u_base				= md_beta .* md.inversion.vx_obs;
md_v_base				= md_beta .* md.inversion.vy_obs;
